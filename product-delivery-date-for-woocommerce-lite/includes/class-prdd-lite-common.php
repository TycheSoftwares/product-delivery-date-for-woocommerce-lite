	...
	public static function prdd_lite_get_product_id( $product_id ) {
		...
		if ( '' === $duplicate_of && null === $duplicate_of ) {
			...
			if ( isset( $post_time->post_date ) ) {
				$results_post_id = $wpdb->get_results( $wpdb->prepare( 'SELECT ID FROM `%s` WHERE post_date = %s ORDER BY ID LIMIT 1', $wpdb->prefix . 'posts', $post_time->post_date ) ); // WPCS: db call ok, WPCS: cache ok.
