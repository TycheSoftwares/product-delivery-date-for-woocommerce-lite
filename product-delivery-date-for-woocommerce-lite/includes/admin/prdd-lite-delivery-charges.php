...
if ( !class_exists( 'prdd_lite_delivery_price' ) ) {
    ...
    class prdd_lite_delivery_price {
        ...
        function prdd_load_ajax_special_delivery_price_block() {
		 ...
		 * @since 2.0
		 */
        function prdd_lite_delivery_price_show_field_settings( $product_id ) {
            global $post, $wpdb;
			?>
           	<div id="special_delivery_page">
			    <table class='form-table'>
                    <tr id="special_delivery_price_row">
                        <th width="35%">
                            <label for="special_delivery_price_block"><?php _e( 'Select Days:', 'woocommerce-prdd' ); ?></label>
                        </th>
                        <td width="5%">&nbsp;</td>
                        <th style="width:175px;">
                            <label for="special_delivery_date"><?php _e( 'Select Date:', 'woocommerce-prdd' ); ?></label>
                        </th>
                        <td width="1%">&nbsp;</td>
                        <th width="20%">
                            <label for="special_delivery_price"><?php _e( 'Delivery Charges:', 'woocommerce-prdd' ); ?></label>
                        </th>
                    </tr>
                    <tr>
                        <td>
                            <select name="special_delivery_weekday" id="special_delivery_weekday" disabled readonly>
                                <option value=""><?php _e( 'Select Weekday', 'woocommerce-prdd-lite' ); ?></option>
                            </select>
                            <img class="help_tip" width="16" height="16" data-tip="<?php _e( 'Select weekday to set delivery charge for', 'woocommerce-prdd-lite' ); ?>" src="<?php echo plugins_url() ;?>/woocommerce/assets/images/help.png"/>
                        </td>
                        <td width="5%">&nbsp;</td>
                        <td>
                            ...
                            <img class="help_tip" width="16" height="16" data-tip="<?php _e( 'Select date to set delivery charge for', 'woocommerce-prdd-lite' ); ?>" src="<?php echo plugins_url() ;?>/woocommerce/assets/images/help.png"/>
                        </td>
                        <td width="1%">&nbsp;</td>
                        <td>
                            <input type="text" name="special_delivery_price" id="special_delivery_price" disabled readonly style="width: 75px;"> 
                            <img class="help_tip" width="16" height="16" data-tip="<?php _e( 'Set delivery charge for the selected day / date', 'woocommerce-prdd-lite' ); ?>" src="<?php echo plugins_url() ;?>/woocommerce/assets/images/help.png"/>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <input type="button" class="button-primary" value="<?php _e( 'Add Delivery Charges', 'woocommerce-prdd' ); ?>" id="save_special_delivery" disabled readonly></input>
		 ...
		 * @since 1.0
		 */
		function prdd_save_special_delivery_price() {
            global $wpdb;
            if( isset( $_POST[ 'bulk_products' ] ) ) {
            	$post_id = $_POST[ 'bulk_products' ];
            } else {
			...
			if( is_array( $delivery_special_prices ) && $delivery_special_price_cnt > 0 ) { 
                foreach ( $delivery_special_prices as $key => $value ) { 
                    ...
                    } else if ( $value[ 'delivery_special_date' ] != "" ){
		...
		function prdd_delete_all_special_delivery() {
            $post_id = $_POST[ 'post_id' ];
		...
		function get_price( $product_id, $delivery_date ) {
            $weekdays = prdd_get_delivery_arrays( 'prdd_weekdays' );
			...
			if( is_array( $delivery_special_prices ) && count( $delivery_special_prices ) > 0 ){
                foreach( $delivery_special_prices as $key => $values ){
					...
					$delivery_day =  date( "l", mktime( 0, 0, 0, $month, $day, $year ) );
		...
		function special_delivery_add_to_cart( $cart_arr, $product_id, $variation_id, $cart_item_meta ) {
			global $woocommerce;
			...
			if ( array_key_exists( 'composite_parent', $cart_item_meta ) && $cart_item_meta[ 'composite_parent' ] !== '' ) {
	            ...
	            if( !( isset( $cart_arr[ 'time_slot_charges' ] ) ) ) {
	                if( isset( $special_delivery_price ) && $special_delivery_price != "" && $special_delivery_price != 0 ) {
		...
		public static function prdd_get_special_delivery_price( $product_id, $delivery_date, $date_array ) {
			...
			if( is_array( $delivery_special_prices ) && count( $delivery_special_prices ) > 0 && 'Yes' == $include_price ) {
                ...
                $delivery_day = date( 'w', strtotime( $date ) );
				...
				foreach( $delivery_special_prices as $key => $values ) {
                    if ( isset( $values[ 'delivery_special_weekday' ] ) && $values[ 'delivery_special_weekday' ] == $delivery_weekday ) {
                        $special_delivery_price = $values[ 'delivery_special_price' ];
                    } else if( isset( $values[ 'delivery_special_date' ] ) && $values[ 'delivery_special_date' ] == $date ){
                        $special_delivery_price = $values[ 'delivery_special_price' ];
                    }
                }
            }
		...
		function get_special_delivery_cart_item_from_session( $cart_item, $values ) {
