    ...
    public static function prdd_lite_woocommerce_prdd_history_page() {
        ...
        if ( $action == 'history' || $action == '' ) {
        	...
        	<div class="wrap">
                ...
                <h2><?php _e( 'All Deliveries', 'woocommerce-prdd-lite' ); ?></h2>

        		<?php do_action( 'prdd_lite_page_top' ); ?>
        		<form id="prdd-lite-view-deliveries" method="get" action="<?php echo admin_url( 'admin.php?page=woocommerce_prdd_lite_history_page' ); ?>">

        			<p id="prdd_lite_add_order">
        			    <button class="button-secondary" disabled>
    						<?php _e ( 'Calendar View', 'woocommerce-prdd-lite' ); ?>
						</button>
                        ...
                        if ( !isset( $_GET[ 'prdd_view' ] ) ) {
                            ...
                            <button style="float:right;" class="button-secondary" disabled><?php _e( 'Print', 'woocommerce-prdd-lite' ); ?></button>
                            <button style="float:right;" class="button-secondary" disabled><?php _e( 'CSV', 'woocommerce-prdd-lite' ); ?></button>
                            <br>
...
$view_deliveries_lite = new view_deliveries_lite();