...
class prdd_lite_delivery_settings {
    ...
    public static function prdd_lite_months_callback( $args ) {
        ...
        $no_months_1 = $no_months_2 = "";
        
        if ( $prdd_months == 1 ) {
            $no_months_1 = "selected";
            $no_months_2 = "";
        } elseif ( $prdd_months == 2 ) {
            $no_months_2 = "selected";
            $no_months_1 = "";
        }

        echo '<select id="prdd_lite_months" name="prdd_lite_months">
    ...
    public static function prdd_lite_calendar_day_callback( $args ) {
        ...
        foreach ( $days as $key => $value ) {
            ...
            echo "<option value='$key' $sel>" . __( $value, 'woocommerce-prdd' ) . "</option>";
        }
        echo '</select>';
    ...
    public static function prdd_lite_theme_callback( $args ) {
        ...
        echo '<input type="hidden" name="prdd_lite_theme" id="prdd_lite_theme" value="' . $prdd_themes . '">';
            ...
            jQuery( "#prdd_lite_new_switcher" ).themeswitcher({
                onclose: function() {
                    ...
                    jQuery( "<link/>", {
                        rel: "stylesheet",
                        type: "text/css",
                        href: "' . plugins_url() . '/product-delivery-date-for-woocommerce-lite/css/datepicker.css"
                    }).appendTo( "head" );
                },
                imgpath: "' . plugins_url() . '/product-delivery-date-for-woocommerce-lite/images/",
    ...
    public static function prdd_lite_enable_rounding_callback( $args ) {
        ...
        $rounding = "";
        if ( isset( $prdd_enable_rounding ) && $prdd_enable_rounding == 'on' ) {
            $rounding = 'checked';
        }
    ...
    /*public static function ts_calendar_export_custom_field_type_callback( $args ) {
        $gcal_export_custom_field_type = ts_get_option( 'ts_calendar_export_custom_field_type' );
        $custom_order_meta = '';
        $custom_product_meta = '';
        if( $gcal_export_custom_field_type == 'custom_order_meta' ) {
            $custom_order_meta = 'selected';
            $custom_product_meta = '';
        } else if( $gcal_export_custom_field_type == 'custom_product_meta' ) {
            $custom_order_meta = '';
            $custom_product_meta = 'selected';
        }

        echo '<select id="ts_calendar_export_custom_field_type" name="ts_calendar_export_custom_field_type">
            <option value="custom_order_meta" ' . $custom_order_meta . '>' . __( 'Custom Order meta', 'ts' ) . '</option>
            <option value="custom_product_meta" ' . $custom_product_meta . '>' . __( 'Custom Product meta', 'ts' ) . '</option>
        </select>';

        $html = '<label for="ts_calendar_export_custom_field_type"> ' . $args[0] . '</label>';
        echo $html;   
    }

    public static function ts_calendar_export_custom_field_callback( $args ) {
        $gcal_export_custom_field = ts_get_option( 'ts_calendar_export_custom_field' );
        echo '<input id="ts_calendar_export_custom_field" name="ts_calendar_export_custom_field" value="' . $gcal_export_custom_field . '" type="text"/>';
        $html = '<label for="ts_calendar_export_custom_field"> ' . $args[0] . '</label>';
        echo $html;   
    }*/
    ...
    public static function ts_calendar_sync_integration_mode_callback( $args ) {
        echo '<input type="radio" name="ts_calendar_sync_integration_mode" id="ts_calendar_sync_integration_mode" value="directly" disabled/>' . __( 'Sync Automatically', 'woocommerce-prdd' ) . '&nbsp;&nbsp;
            <input type="radio" name="ts_calendar_sync_integration_mode" id="ts_calendar_sync_integration_mode" value="manually" disabled/>' . __( 'Sync Manually', 'woocommerce-prdd' ) . '&nbsp;&nbsp;
            <input type="radio" name="ts_calendar_sync_integration_mode" id="ts_calendar_sync_integration_mode" value="disabled" disabled/>' . __( 'Disabled', 'woocommerce-prdd' );
