...
class PRDD_Lite_View_Deliveries_Table extends WP_List_Table {
	...
	public function get_views() {
		...
		$views = array(
				'all'		=> sprintf( '<a href="%s"%s>%s</a>', remove_query_arg( array( 'status', 'paged' ) ), $current === 'all' || $current == '' ? ' class="current"' : '', __( 'All', 'woocommerce-prdd' ) . $total_count ),
				'future'	=> sprintf( '<a href="%s"%s>%s</a>', add_query_arg( array( 'status' => 'future', 'paged' => FALSE ) ), $current === 'future' ? ' class="current"' : '', __( 'Deliveries From Today Onwards', 'woocommerce-prdd' ) . $future_count ),
				'today_delivery'	=> sprintf( '<a href="%s"%s>%s</a>', add_query_arg( array( 'status' => 'today_delivery', 'paged' => FALSE ) ), $current === 'today_delivery' ? ' class="current"' : '', __( 'Today\'s Deliveries', 'woocommerce-prdd' ) . $today_delivery_count ),
				'tomorrow_delivery'	=> sprintf( '<a href="%s"%s>%s</a>', add_query_arg( array( 'status' => 'tomorrow_delivery', 'paged' => FALSE ) ), $current === 'tomorrow_delivery' ? ' class="current"' : '', __( 'Tomorrow\'s Deliveries', 'woocommerce-prdd' ) . $tomorrow_delivery_count )		);
	...
	public function prdd_lite_count_deliveries( $args ) {
		...
		if ( $start_date != '' && $end_date != '' && $start_date != '1970-01-01' && $end_date != '1970-01-01' ) {
		    ...
		    $query_get_order_item_ids = "SELECT order_item_id FROM `" . $wpdb->prefix . "woocommerce_order_itemmeta` WHERE meta_key = %s";
		    $get_order_item_ids = $wpdb->get_results( $wpdb->prepare( $query_get_order_item_ids, '_prdd_lite_date' ) );
		        ...
		        foreach ( $get_order_item_ids as $item_key => $item_value ) {
		            ...
		            if( isset( $order_post_status ) && ( $order_post_status != '' ) && ( $order_post_status != 'wc-cancelled' ) && ( $order_post_status != 'wc-refunded' ) && ( $order_post_status != 'trash' ) && ( $order_post_status != 'wc-failed' ) ) {
                        $query_get_dates = "SELECT meta_value, meta_key FROM `".$wpdb->prefix."woocommerce_order_itemmeta`
														WHERE meta_key = %d
														AND order_item_id = %d";
                        $get_dates[] = $wpdb->get_results( $wpdb->prepare( $query_get_dates, '_prdd_lite_date' , $item_value->order_item_id ) );
		            }
		        }
		    }
		}
	...
	public function deliveries_data() { 
		...
		if ( isset( $_GET[ 'status' ] ) && $_GET[ 'status' ] == 'future' ) {
			$query_get_id = "SELECT t1.order_id, GROUP_CONCAT( t2.meta_key, '=' ,t2.meta_value SEPARATOR '&' ) AS meta_values FROM  `" . $wpdb->prefix . "woocommerce_order_items` AS t1, `" . $wpdb->prefix . "woocommerce_order_itemmeta` AS t2 WHERE t2.meta_key IN (%s,%s) AND t2.order_item_id IN ( SELECT order_item_id FROM `" . $wpdb->prefix . "woocommerce_order_itemmeta` WHERE meta_key = %s AND meta_value >= %s ) AND t1.order_item_id = t2.order_item_id AND t1.order_id IN ( SELECT ID FROM `" . $wpdb->prefix . "posts` WHERE post_type = 'shop_order' AND post_status NOT IN ( 'wc-cancelled', 'wc-refunded', 'trash', 'wc-failed', '' ) ) GROUP BY t1.order_item_id HAVING COUNT( DISTINCT t2.meta_key ) = 3 OR COUNT( DISTINCT t2.meta_key ) = 2 ORDER BY t1.order_id DESC $limit";

            $get_order_item_id = $wpdb->get_results( $wpdb->prepare( $query_get_id, '_product_id', '_prdd_lite_date', '_prdd_lite_date', $current_date ) );

        } else if ( isset( $_GET[ 'status' ] ) && $_GET[ 'status' ] == 'today_delivery' ) {
        	$query_get_id = "SELECT t1.order_id, GROUP_CONCAT( t2.meta_key, '=' ,t2.meta_value SEPARATOR '&' ) AS meta_values FROM  `" . $wpdb->prefix . "woocommerce_order_items` AS t1, `" . $wpdb->prefix . "woocommerce_order_itemmeta` AS t2 WHERE t2.meta_key IN (%s,%s) AND t2.order_item_id IN ( SELECT order_item_id FROM `" . $wpdb->prefix . "woocommerce_order_itemmeta` WHERE meta_key = %s AND meta_value = %s ) AND t1.order_item_id = t2.order_item_id AND t1.order_id IN ( SELECT ID FROM `" . $wpdb->prefix . "posts` WHERE post_type = 'shop_order' AND post_status NOT IN ( 'wc-cancelled', 'wc-refunded', 'trash', 'wc-failed', '' ) ) GROUP BY t1.order_item_id HAVING COUNT( DISTINCT t2.meta_key ) = 3 OR COUNT( DISTINCT t2.meta_key ) = 2 ORDER BY t1.order_id DESC $limit";

            $get_order_item_id = $wpdb->get_results( $wpdb->prepare( $query_get_id, '_product_id', '_prdd_lite_date', '_prdd_lite_date', $current_date ) );
        } else if ( isset( $_GET[ 'status' ] ) && $_GET[ 'status' ] == 'tomorrow_delivery' ) {
            $query_get_id = "SELECT t1.order_id, GROUP_CONCAT( t2.meta_key, '=' ,t2.meta_value SEPARATOR '&' ) AS meta_values FROM  `" . $wpdb->prefix . "woocommerce_order_items` AS t1, `" . $wpdb->prefix . "woocommerce_order_itemmeta` AS t2 WHERE t2.meta_key IN (%s,%s) AND t2.order_item_id IN ( SELECT order_item_id FROM `" . $wpdb->prefix . "woocommerce_order_itemmeta` WHERE meta_key = %s AND meta_value = %s ) AND t1.order_item_id = t2.order_item_id AND t1.order_id IN ( SELECT ID FROM `" . $wpdb->prefix . "posts` WHERE post_type = 'shop_order' AND post_status NOT IN ( 'wc-cancelled', 'wc-refunded', 'trash', 'wc-failed', '' ) ) GROUP BY t1.order_item_id HAVING COUNT( DISTINCT t2.meta_key ) = 3 OR COUNT( DISTINCT t2.meta_key ) = 2 ORDER BY t1.order_id DESC $limit";

            $get_order_item_id = $wpdb->get_results( $wpdb->prepare( $query_get_id, '_product_id', '_prdd_lite_date', '_prdd_lite_date', $tomorrow_date ) );
        } else {
		    ...
		    $get_order_item_id = $wpdb->get_results( $wpdb->prepare( $query_get_id, '_prdd_lite_date', '_product_id' ) );
        }
        ...
        foreach ( $get_order_item_id as $key => $value ) {
			...
			foreach( $meta_values as $mk => $mv ) {
				$values = explode( "=", $mv );
				//print_r( $values );
				if( $values[0] == '_prdd_lite_date' ) {
				    $date = $values[1];
				}
				
				if ( $values[0] == '_product_id' ) {
				    $product_id = $values[1];
				}
			}
    	...
    	if( isset( $_GET[ 'orderby' ] ) && $_GET[ 'orderby' ] == 'delivery_date' ) {
	 ...
	 * @since  1.1
	 */
	function prdd_lite_class_delivery_date_asc( $value1, $value2 ) {
	 ...
	 * @since  1.1
	 */
	function prdd_lite_class_delivery_date_dsc( $value1, $value2 ) {
	   if( isset( $value1->delivery_date ) && isset( $value2->delivery_date ) ) {
    	    $date_two            = $date_one = '';
    	    $value_one           = $value1->delivery_date;
    	    $value_two           = $value2->delivery_date;
